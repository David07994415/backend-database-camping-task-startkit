...
select U.id,C.id,C.credit_amount,C.price FROM "USER" U,"CREDIT_PACKAGE" C  WHERE U.name = '王小明' and C.name='14 堂組合包方案';

INSERT INTO "CREDIT_PURCHASE" (user_id,credit_package_id ,purchased_credits,price_paid)
select U.id,C.id,C.credit_amount,C.price FROM "USER" U,"CREDIT_PACKAGE" C  WHERE U.name = '王小明' and C.name='21 堂組合包方案';

INSERT INTO "CREDIT_PURCHASE" (user_id,credit_package_id ,purchased_credits,price_paid)
select U.id,C.id,C.credit_amount,C.price FROM "USER" U,"CREDIT_PACKAGE" C  WHERE U.name = '好野人' and C.name='14 堂組合包方案';
...
((SELECT c.id FROM "COACH" C JOIN "USER" U ON U.id = C.user_id WHERE U.name = '肌肉棒子'),(SELECT id FROM "SKILL" WHERE name = '瑜伽')),
((SELECT c.id FROM "COACH" C JOIN "USER" U ON U.id = C.user_id WHERE U.name = 'Q太郎'),(SELECT id FROM "SKILL" WHERE name = '有氧運動')),
((SELECT c.id FROM "COACH" C JOIN "USER" U ON U.id = C.user_id WHERE U.name = 'Q太郎'),(SELECT id FROM "SKILL" WHERE name = '復健訓練'));
...
SELECT U.id,S.id,'重訓基礎課','2024-11-25 14:00:00','2024-11-25 16:00:00',10,'https://test-meeting.test.io'
FROM "USER" U 
JOIN "SKILL" S on 1=1
WHERE U.name='李燕容' AND S.name ='重訓';
...
(
    ...
    (SELECT C.id FROM "COURSE" as C JOIN "USER" as U ON C.user_id =U.id WHERE U.name = '李燕容' LIMIT 1),'2024-11-24 16:00:00','即將授課'
);
    -- 2. 新增： `好野人` 預約 `李燕容` 的課程
        -- 1. 預約人設為 `好野人`
        -- 2. 預約時間`booking_at` 設為2024-11-24 16:00:00
...
AND course_id = (SELECT C.id FROM "COURSE" as C JOIN "USER" AS U ON U.id = C.user_id  WHERE U.name = '李燕容' );
...
(
    (SELECT id FROM "USER" WHERE name = '王小明'),
    (SELECT C.id FROM "COURSE" as C JOIN "USER" as U ON C.user_id =U.id WHERE U.name = '李燕容' LIMIT 1),'2024-11-24 17:10:25','即將授課'
);
...
WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明') AND status!='課程已取消';
...
SELECT CP.user_id,(CP.total-CB.total) AS remaining_credit
FROM 
(SELECT user_id,SUM(purchased_credits) AS total FROM "CREDIT_PURCHASE"
WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明')
...
S.name AS "專長名稱"
FROM "SKILL" AS S
JOIN "COACH_LINK_SKILL" AS CS ON CS.skill_id = S.id
JOIN "COACH" AS C ON C.id = CS.coach_id
...
SELECT S.name,COUNT(*) as coach_total
FROM "COACH_LINK_SKILL" AS CS
JOIN "SKILL" AS S ON CS.skill_id = S.id
GROUP BY S.name
ORDER BY COUNT(*) DESC
Limit 1;
...
SELECT CPA.name as "組合包方案名稱",COUNT(*) AS "銷售數量"
FROM "CREDIT_PURCHASE" AS CPU
JOIN "CREDIT_PACKAGE" AS CPA ON CPU.credit_package_id = CPA.id
WHERE CPU.created_at::DATE BETWEEN '2024-11-01' AND '2024-11-30'
GROUP BY CPA.name;

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
SELECT SUM(price_paid) AS "總營收" FROM "CREDIT_PURCHASE"
WHERE purchase_at::DATE BETWEEN '2024-11-01' AND '2024-11-30';

-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
SELECT COUNT(distinct(user_id)) as "預約會員人數"
FROM  "COURSE_BOOKING"
WHERE (created_at::DATE BETWEEN '2024-11-01' AND '2024-11-30') and status!='課程已取消';